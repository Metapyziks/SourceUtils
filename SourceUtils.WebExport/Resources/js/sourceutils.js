    ...
    var BspLeaf = (function (_super) {
        ...
        BspLeaf.prototype.getAmbientCube = function (pos, outSamples, callback) {
            ...
            for (var i = 0; i < 6; ++i) {
                ...
                var exp = ((rgbExp >> 24) & 0xff) - 128;
                var mul = Math.pow(2.0, exp);
...
(function (SourceUtils) {
    ...
    var StudioModel = (function (_super) {
        ...
        StudioModel.sampleAmbientCube = function (normal, samples) {
            ...
            rgb.add(sample.x * mul, sample.y * mul, sample.z * mul);
            var r = Math.min(Math.round(rgb.x * 255.0), 255);
            var g = Math.min(Math.round(rgb.y * 255.0), 255);
            var b = Math.min(Math.round(rgb.z * 255.0), 255);
            return r | (g << 8) | (b << 16);
        };
